...
export const Header: React.FC<HeaderProps> = ({ activeTab, onToggleMobileNav }) => {
  ...
  return (
    <header className="w-full h-14 sm:h-16 md:h-18 px-3 sm:px-6 md:px-8 bg-white dark:bg-black border-b border-black/[0.04] dark:border-white/[0.04] flex items-center justify-between gap-2 sm:gap-4 md:gap-6 shrink-0 z-30 relative transition-colors">
      ...
      {/* Left: Active Page Title (Consistent across all pages, no overlapping on mobile) */}
      <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-shrink">
        <button
          onClick={onToggleMobileNav}
          className="md:hidden text-zinc-700 dark:text-zinc-300 hover:text-black dark:hover:text-white p-2 rounded-xl bg-black/[0.05] dark:bg-white/[0.05] cursor-pointer shrink-0 transition-colors"
          aria-label="Open navigation menu"
        >
          <Menu className="w-4 h-4" />
        </button>

        <h1 className="text-base sm:text-2xl md:text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-white truncate whitespace-nowrap">
