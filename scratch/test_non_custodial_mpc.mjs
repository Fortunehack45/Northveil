...
async function runTests() {
  ...
  assert(!walletResult.privateKey && !walletResult.backupSeedPhrase && !walletResult.secret, 'ZERO raw private keys or seed phrases returned');

  const vaultAddress = walletResult.address;

  // 2. Test Autonomous Spending Scopes & Evaluation
  console.log('\n2️⃣ Testing Autonomous Spending Policy Evaluation...');
  // Check default behavior (no active scope)
  ...
  // Activate spending scope
  const { initSupabase } = await import('../api/mpcControlPlaneService.js');
  // Add in-memory scope directly or test via evaluateAutonomousScope
  ...
  // 5. Test Single-Use Approval Token Invalidation
  ...
  assert(isKillSwitchActive(vaultAddress), 'Vault recognized as locked');
  ...
  assert(!scopeUnderLock.inScope && scopeUnderLock.reason?.includes('Emergency Kill Switch'), 'Autonomous scope evaluation rejects when kill switch is active');

  // Deactivate kill switch
  const unlockRes = await deactivateKillSwitch(vaultAddress, 'test_user_01');
  assert(!isKillSwitchActive(vaultAddress), 'Vault unlocked after deactivation');
